  ...
  while (pos >= 0 && pos < MAXVOTES - 1) {
    ...
    switch (cmd) {
      case 'A':
        ...
        locks(SEM_VOTE);
        memcpy(voteinfo->vote[pos].msg, (const char *)tmpstart,
               sizeof(voteinfo->vote[pos].msg) - 1);
      ...
      case 'R':
        if (ouruser->f_admin) {
          ...
          for (i = 1; i < MAXTOTALUSERS; i++) {
            if (voteinfo->vote[pos].answer[i] &&
                (user = finduser(NULL, i, 0))) {
              my_printf("%s: %c\n", user->name, voteinfo->vote[pos].answer[i]);
              freeuser(user);
              if (++linenbr >= rows - 1 && line_more(&linenbr, -1)) break;
            }
          }
          continue;
        }
...
static int makevotemsg(void) {
  ...
  for (;;) {
    if ((uint32_t)(tmpp - tmpstart) > sizeof(vote.msg) - 1) {
    ...
    if (chr < ' ' && (chr != CTRL_D) && chr != BS && chr != TAB && chr != LF &&
        chr != CTRL_X && chr != CTRL_W && chr != CTRL_R) {
      if (invalid++) {
