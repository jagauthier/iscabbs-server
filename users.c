 ...
 */
struct user *finduser(const char *name, uint32_t usernum, int linknum) {
  int32_t lower = 0;
  int32_t upper = 0;
  int32_t mid = 0;
  int32_t cmp = 0;
 ...
 * NULL on failure.
 */
struct user *adduser(const char *name, uint32_t usernum) {
  struct user *user;
  int32_t old;
  int32_t new;
  int32_t free;
  ...
  for (;;) {
    ...
    if ((user = finduser(name, 0, 0))) {
      if (user->usernum || time(0) < user->timeoff + (30 * 60)) {
        errlog("Found user %s with name '%s' num %i timeoff %ld time %ld", name,
               user->name, user->usernum, user->timeoff, time(0));
        user = NULL;
        break;
      } else {
        unlocks(SEM_INDEX);
    ...
    memcpy((void *)&udata->name[new][0], (void *)&udata->name[old][0],
           saveindex * sizeof(int));
    udata->name[new][saveindex] = free;
    memcpy((void *)&udata->name[new][saveindex + 1],
           (void *)&udata->name[old][saveindex],
           (udata->totalusers[old] - saveindex) * sizeof(int));

    /* Check if usernum exists */
    if (finduser(NULL, usernum, 0)) {
      /* This could happen after a crash unless usernum eternal always syncd */
      errlog("Found old usernum %i (%s) in adduser", usernum,
             udata->link[udata->name[old][saveindex]].name);
      break;
    }

    memcpy((void *)&udata->num[new][0], (void *)&udata->num[old][0],
           saveindex * sizeof(int));
    udata->num[new][saveindex] = free;
    memcpy((void *)&udata->num[new][saveindex + 1],
           (void *)&udata->num[old][saveindex],
           (udata->totalusers[old] - saveindex) * sizeof(int));

    msync((char *)udata + sizeof(struct user),
          sizeof(struct userdata) - sizeof(struct user), MS_SYNC);
    udata->which = new;
...
int deleteuser(const char *name) {
  ...
  for (;;) {
    ...
    memcpy((void *)&udata->name[new][0], (void *)&udata->name[old][0],
           saveindex * sizeof(int));
    memcpy((void *)&udata->name[new][saveindex],
           (void *)&udata->name[old][saveindex + 1],
           (udata->totalusers[new] - saveindex) * sizeof(int));
    udata->name[new][udata->totalusers[new]] = 0;

    if (!finduser(NULL, linkptr->usernum, 0)) {
      errlog("Failed to find user %s by usernum %i (should not happen)", name,
             linkptr->usernum);
      break;
    }
    memcpy((void *)&udata->num[new][0], (void *)&udata->num[old][0],
           saveindex * sizeof(int));
    memcpy((void *)&udata->num[new][saveindex],
           (void *)&udata->num[old][saveindex + 1],
           (udata->totalusers[new] - saveindex) * sizeof(int));
 ...
 */
struct user *getuser(const char *name) {
  struct user *user;
...
char *getusername(const uint32_t num, const int how) {
  return (finduser(NULL, num, 0) ? udata->link[savelinknum].name
                                 : (how ? "<Deleted User>" : NULL));
}

/*
 * Returns index to user in udata structure, for use with finduser().
 */
int getuserlink(const struct user *tmpuser) {
 ...
 */
void freeuser(struct user *tmpuser) {
#if 0
  /*
  ...
  if (tmpuser != ouruser &&
      ((index = tmpuser->btmpindex) < 0 ||
       tmpuser->usernum != bigbtmp->btmp[index].usernum) &&
      strcmp(tmpuser->name, "Guest")) {
...
int openuser(void) {
  ...
  return ((int)udata);
#else
  //  if (madvise((void *)udata, sizeof(struct userdata) + sizeof(struct user) *
  //  MAXTOTALUSERS,/* MADV_RANDOM */ 0) < 0)
  //    return(-1);
  return (0);
#endif
}
...
struct userdata *copyuserdata(void) {
  ...
  ucopy = (struct userdata *)mmap(0, sizeof(struct userdata),
...
int backupuserdata(void) {
  ...
  zero = (char *)mmap(0, sizeof(struct user), PROT_READ | PROT_WRITE,
                      MAP_ANONYMOUS | MAP_PRIVATE, -1, 0);
  if (!zero || zero == (char *)-1) {
    return (-1);
  }
  ...
  for (i = 0; i < MAXTOTALUSERS; i++) {
    if (*ucopy->link[i].name && ucopy->link[i].usernum) {
      ...
      if (!up) {
        errlog(
            "backupuserdata(): Couldn't find user name %s (num %i) in slot %d",
            ucopy->link[i].name, ucopy->link[i].usernum, i);
      } else if (up != finduser(NULL, ucopy->link[i].usernum, 0)) {
        errlog(
            "backupuserdata(): Couldn't find user num %i (name %s) in slot %d",
            ucopy->link[i].usernum, ucopy->link[i].name, i);
      } else if (up->usernum != ucopy->link[i].usernum ||
                 strcmp(up->name, ucopy->link[i].name)) {
        errlog(
            "backupuserdata(): Mismatch for user %s num %i in slot %d "
            "('%s'/%i)",
            ucopy->link[i].name, ucopy->link[i].usernum, i, up->name,
            up->usernum);
      } else {
  ...
  if (count != ucopy->totalusers[ucopy->which]) {
    errlog("backupuserdata(): backed up %d users, total should be %d", count,
           ucopy->totalusers[ucopy->which]);
  }
