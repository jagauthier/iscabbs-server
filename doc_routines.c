  ...
  for (i = 0; i < MSGSPERRM; i++) {
    ...
    if (room->num[i] > 0L) {
      ...
      if (room->num[i] > ouruser->lastseen[curr] &&
          (curr != MAIL_RM_NBR ||
           ouruser->mr[MAILMSGS - MSGSPERRM + i].pos > 0)) {
        new ++;
      }
    }
  }
...
-----------------------------------------------------------------------*/
void knrooms(struct user *tmpuser) {
  int16_t i;
  int8_t limit = 24;
  int16_t linenbr;
  int16_t newlength;
  ...
  for (rm_nbr = 0; rm_nbr < MAXROOMS; ++rm_nbr) {
    if ((msg->room[rm_nbr].flags & QR_INUSE) &&
      ...
      checked_snprintf(tmpstr, sizeof(tmpstr), " %d\056%s>  ", rm_nbr,
                       msg->room[rm_nbr].name);
      while (strlen(tmpstr) % limit) strcat(tmpstr, " ");
      newlength = oldlength + strlen(tmpstr);

      if (newlength > MARGIN) {
        my_putchar('\n');
        if (++linenbr >= rows - 1 && line_more(&linenbr, -1)) return;
        oldlength = 1;
      }
 ...
 ************************************************************/
int line_more(int16_t *nbr, int16_t percent) {
  int chr;
  ...
  for (;;) {
    ...
    if (client) {
      for (;;) {
        ...
        if (chr != 17 || !client || numposts <= 0) break;
      }
    } else {
 ...
 *********************************************************************/
void fr_post(uint8_t rm, int32_t msgnum, int32_t pos, int32_t mmhi,
             struct user *tmpuser) {
  uint8_t i;
 ...
 ***************************************************************************/
void storeug(int32_t *uglastmsg, int32_t *ugtemp) {
  *uglastmsg = *ugtemp;
 ...
 * ungoto
 **********************************************************************/
void ungoto(int prev, int32_t *uglastmsg, int32_t *ugtemp) {
  if (prev == TWILIGHTZONE) return;
...
char *formtime(int how, time_t timetoform) {
  ...
  if (ouruser->f_ampm) {
    hr = !tm->tm_hour ? 12 : tm->tm_hour > 12 ? tm->tm_hour - 12 : tm->tm_hour;
  ...
  switch (how) {
    ...
    case 2:
      ...
      checked_snprintf(tstring, sizeof(tstring), "%s %d, %d %d:%02d%s",
                       months[tm->tm_mon], tm->tm_mday, 1900 + tm->tm_year, hr,
                       tm->tm_min, stamp);
      break;
    ...
    case 3:
      ...
      checked_snprintf(tstring, sizeof(tstring), "%d/%d/%02d %d:%02d%s",
                       tm->tm_mon + 1, tm->tm_mday, tm->tm_year % 100, hr,
                       tm->tm_min, stamp);
      break;

    case 4:
      /* 1:23 PM */
      checked_snprintf(tstring, sizeof(tstring), "%d:%02d%s", hr, tm->tm_min,
                       stamp);
      break;
    ...
    case 6: /* X message */
      checked_snprintf(tstring, sizeof(tstring), "%d:%02d%s on %s %d, %d", hr,
                       tm->tm_min, stamp, months[tm->tm_mon], tm->tm_mday,
                       1900 + tm->tm_year);
      break;
