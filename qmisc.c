...
void do_ring(void) {
  ...
  /*
    if (q->limit < 0)
    {
      ...
      if (!q->cpuuser)
        q->startable = -q->limit - (int)pst->psd_avg_1_min * 2 -
  (int)pst->psd_avg_15_min * 2; else
      {
        q->startable = (-q->limit - ((int)pst->psd_avg_1_min * 2 +
  (pst->psd_cpu_time[0] + pst->psd_cpu_time[2] - q->cpuuser - q->cpusys) /
  ((q->lastcheck && (q->t - q->lastcheck)) ? (q->t - q->lastcheck) : 1))) / 2;
        if (q->qp > -q->limit / 8 && q->startable > -q->limit / 8 &&
  q->startable > -q->limit - (int)pst->psd_avg_15_min * 4) q->startable =
  -q->limit - (int)pst->psd_avg_15_min * 4; #if 1 Temporary hack while whip ==
  liquefy if (bigbtmp->users > 4100) q->startable = MIN(q->startable, q->qp /
  2); if (bigbtmp->users > 4200) q->startable = MIN(q->startable, q->qp / 16);
  #endif
        q->oldforks = q->forks;
      }
      q->lastcheck = q->t;
...
void do_reread(void) {
  ...
  q->hellolen = 0;
  while ((size_t)(q->hellolen + 1) < sizeof(q->hello)) {
    if ((c = getc(f)) == '\n') {
...
void do_quit(void) {
  syslog(LOG_INFO, "users %i, queue %d, limit %d, lockout %d",
         q->forks - q->reaps, q->qp, q->limit, q->lockout);
  syslog(LOG_INFO, "forks %i, maxqueue %d", q->forks, q->maxqp);
  syslog(LOG_INFO, "admins %i, upgrades %i, users %i", q->aidewiz, q->upgrade,
         q->nonupgrade);
  _exit(0);
}
...
static void checkauth(int x) {
  ...
  for (i = MAILMSGS - 1, j = 0;
       i >= 0 && u->lastseen[MAIL_RM_NBR] < u->mr[i].num; i--) {
    if (u->mr[i].pos > 0) {
      if (j++ < 0) {
        j++;
      }
  ...
  if (q->qt[x].login == 5) {
    ...
    if (c == '\r' || c == '\n') {
      ...
      q->qt[x].login = 10;
               ...
               q->qt[x].login < 29) {
      if (c >= 'a' && c <= 'z' &&
          (q->qt[x].login == 10 || q->qt[x].name[q->qt[x].login - 11] == ' ')) {
        c -= 32;
      }
      d = c;
      ssend(x, &d, 1);
...
void logfatal(const char *error) {
