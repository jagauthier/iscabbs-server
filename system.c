...
struct user *login_user(const char *name, const char *passwd) {
  struct user *up;
  ...
  if (guest) {
    ...
    tmpuser = (struct user *)mmap(
        0, sizeof(struct user), PROT_READ | PROT_WRITE,
        MAP_ANONYMOUS /* | MAP_VARIABLE */ | MAP_PRIVATE, -1, 0);
    if (!tmpuser || tmpuser == (struct user *)-1) return (NULL);
...
/*
 * change_password (what_user, old_pass, new_pass)
 *
 * Returns TRUE if old password correct
 */

void change_password(struct user *up, const char *new) {
  time_t salt;
  char saltc[4];
  uint8_t i;
  char c;
...
int new_user(void) {
  ...
  /*
    if (!yesno(-1))
    {
      my_printf("\n\nOK, you'll be put back at the regular login prompt where
  ...
  */
  ...
  for (;;) {
    ...
    int breakflag = NO;
    {
      my_printf("\nPlease choose a name, up to %d letters long: ",
                MAXALIAS - 1);
      flush_input(0);
      name = get_name("", 1);
      check_quit(name);
      ...
      name2[i] = 0;
      if (!strcmp(name2, "new") || strstr(name2, "sysop") ||
          strstr(name2, "moderator") ||
          /*strstr(name2, "isca") || !strcmp(name2, "guest") ||*/
          (tmpuser = getuser(name))) {
        if (tmpuser) {
          freeuser(tmpuser);
          tmpuser = NULL;
    ...
    for (;;) {
      flush_input(0);
      get_string("Choose a password, at least six characters long: ", -8, pas,
                 -1);
      check_quit(pas);
      if ((j = strlen(pas)) < 6) {
        my_printf(
    ...
    for (;;) {
      flush_input(0);
      get_string("\nYour full name (first AND last!): ", MAXNAME, real_name,
                 -1);
      check_quit(real_name);
      for (i = 0, j = strlen(real_name); i < j; i++) {
        if (!isalpha(real_name[i]) && real_name[i] != ' ' &&
            real_name[i] != '-' && real_name[i] != '.' && real_name[i] != ',') {
          break;
        }
      }
      if (!j) {
        ...
        if (!yesno(-1)) {
          my_printf(
              "\n\nOK, we'll let you select a different name for "
              "yourself...\n\n");
          name = 0;
        }
        break;
    ...
    for (;;) {
      ...
      if (!*mail) {
        my_printf(
            "\nYou MUST enter a valid e-mail address to be allowed to use this "
            "BBS!  If you do\nnot have one, we are sorry, you cannot be "
            "allowed to use this BBS!\n\n");
      } else if (!strncmp(mail, "in%", 3) || !strncmp(mail, "IN%", 3) ||
                 !strncmp(mail, "smtp%", 5) || !strncmp(mail, "SMTP%", 5)) {
        my_printf(
            "\nIf your e-mail address begins with IN%% or SMTP%%, please "
            "provide it without that\nheader and with any \"'s (quotation "
            "marks) removed.  It should be of the form\naccountname@sitename.  "
            "An example would be bbs@bbs.isca.uiowa.edu\n\n");
      } else if (strchr(mail, '"')) {
        my_printf(
            "\nA valid e-mail address does not have any \"'s (quotation marks) "
            "in it.  Please\nprovide your e-mail address absent any quotation "
            "marks.\n\n");
      } else if (!(p = (char *)strchr(mail, '@')) || !strchr(mail, '.') ||
                 strchr(mail, ' ')) {
        my_printf(
            "\nThat is not a valid e-mail address, please try again.\n\n");
      } else if (isdigit(p[1])) {
        my_printf(
            "\nYou must use a name, not a numeric address, following the '@' "
            "in your e-mail\naddress.  Please try again.\n\n");
      } else if (!strcasecmp(p, "@bbs.isca.uiowa.edu") ||
                 !strcasecmp(p, "@whip.isca.uiowa.edu")) {
        my_printf("\nThat is NOT your e-mail address.  Please try again.\n\n");
      } else if (!strcasecmp(p, "@chop.isca.uiowa.edu") ||
                 !strcasecmp(p, "@panda.isca.uiowa.edu") ||
                 !strcasecmp(p, "@panda.uiowa.edu")) {
        my_printf(
            "\nSorry, that e-mail address is not valid for validation "
            "purposes.  You cannot\nuse an e-mail address from the Panda "
        ...
        if (!strcasecmp(p, "@anon") && c == '.') {
          my_printf(
              "\nYou may not use an anonymous site as your e-mail "
              "address!\n\n");
        } else {
          p[5] = c;
          break;
    ...
    my_printf(
        "\n\nOK, now we've got everything we need.  You can now review what "
        "you have\nprovided and be given a chance to correct yourself if you "
        "made a mistake.\n\nYou entered:\n\nName: %s\nStreet: %s\nCity: "
        "%s\nState/Country: %s\nZIP/mail code: %s\nPhone: %s\n\nE-mail "
        "address: %s\n\n\n",
  ...
  ouruser->an_name = ouruser->an_addr = ouruser->an_location =
      ouruser->an_phone = ouruser->an_mail = anonymous;
  ouruser->timescalled = 1;
  ouruser->f_ownnew = ouruser->f_novice = ouruser->f_ansi = ouruser->f_newbie =
      ouruser->f_nobeep = 1;
  ...
  {
    char *logmsg = my_sprintf(NULL, "NEWUSER %s%s%s", ouruser->loginname,
                              *ouruser->loginname ? "@" : "", ouruser->remote);
    logevent(logmsg);
    free(logmsg);
  }
