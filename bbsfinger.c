...
// Function to capitalize all words in a sentence and remove trailing whitespace.
void capitalize_and_trim(char *str) {
    bool newWord = true;
    int endOfText = 0;  // This will be used to mark the end of non-whitespace text.
    ...
    for (int i = 0; str[i] != '\0'; ++i) {
        ...
        if (newWord && isalpha((unsigned char)str[i])) {
            ...
            newWord = false;
            endOfText = i;  // Update the endOfText marker.
        } else if (str[i] == ' ') {
            newWord = true;
...
void handle_client(int client_socket, struct sockaddr_in client_address) {
    ...
    if (check_ip_blocklist(client_address.sin_addr.s_addr)) {
        ...
        errlog("Blocked connection from %s", inet_ntoa(client_address.sin_addr));
        close(client_socket);
        return;
    }
