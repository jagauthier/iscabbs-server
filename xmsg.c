...
static void sendx(struct btmp *buser, struct user *touser,
                  char send_string[][80], int override);

static int syself_ok(const char *name);

static int displayx(int32_t pos, int num, time_t *t, int32_t *prev,
                    int32_t *next) {
    char nstr[12];
    ...
    if (msg->xcurpos + (msg->xmsgsize >> 6) >
        (pos < msg->xcurpos ? pos + msg->xmsgsize : pos)) {
        errlog("X database scrollover pos is %08x, curpos is %08x", pos,
               msg->xcurpos);
        return (1);
    }
    ...
    if (xh->snum != ouruser->usernum) {
        ...
        if (xh->rnum != ouruser->usernum) {
            errlog("X database user not send/recv, got %08x and %08x", xh->snum,
                   xh->rnum);
            return (-1);
        }
    ...
    if (t && *t && xh->time > *t + 300) {
        errlog(
            "X time sequence invalid, got %08lx wanted %08lx, args were %08x, %d, "
            "%08x, %08x",
            xh->time, *t, pos, num, prev ? *prev : -1, next ? *next : -1);
        return (1);
    } else if (t) {
        *t = xh->time;
    }
    ...
    my_printf("\n%s %s %s%s %s at %s %s\n",
              ...
              xh->type == X_QUESTION ? "Question" : "Message", nstr,
              sender > 0 ? "to" : "from", name, formtime(6, xh->time),
              sender > 0 ? "---" : (xh->type == X_QUESTION ? "%%%" : "***"));
    s = (char *)(void *)(xh + 1);
    while (*s) {
        s += sprintf(X, "%c%s\n",
...
void checkx(int resetnox) {
    ...
    if (ouruser && ouruser->xseenpos) {
        ...
        while (ouruser->xseenpos) {
            if (!(i = displayx(ouruser->xseenpos, xmsgnum, NULL, NULL,
                               &ouruser->xseenpos))) {
                xmsgnum++;
            }
...
void express(int which) {
    ...
    if (ouruser->f_newbie) {
        /*  Commenting this out because it duplicates the newuseraccess helpfile
            ...
            my_printf("\nSorry, you are not permitted to send X messages until your
    ...
    if (ouruser->f_twit) {
        my_printf(
            "\nSorry, you may not send X messages while your BBS privileges are "
    ...
    if (which < 0) {
        if (!*xreply) {
            colorize(BOLD_RED "Who's that now?\n");
            return;
        }
        strcpy(to, xreply);
        my_printf("Reply eXpress\nRecipient: %s\n", to);
        if (!(p = getuser(to)) || p->f_invisible) {
            colorize(BOLD_RED "There is no user %s on this BBS.\n", to);
            if (p) {
                freeuser(p);
            }
            *to = 0;
    ...
    } else if (which >= 0 && which < 10) /* QuickX */
    {
        ...
        if (!p || p->f_invisible) {
            colorize(BOLD_RED "That user no longer exists.\n");
            if (p) {
                freeuser(p);
            }
            locks(SEM_USER);
    ...
    if (!(buser = is_online(&tuser, p, NULL))) {
        freeuser(p);
        colorize(BOLD_RED "User is not online.\n");
        *to = 0;
        flush_input(1);
        return;
    }
    ...
    if (0 && p->f_twit) {
        freeuser(p);
        colorize(BOLD_RED "Sorry, you can't X a twit!\n");
        *to = 0;
        flush_input(1);
        return;
    }
    ...
    if (p->f_newbie && !(ouruser->f_elf && mybtmp->elf) && !ouruser->f_admin) {
        ...
        if (ouruser->f_elf) {
            colorize(
                BOLD_RED
                "You can only X new or unvalidated users if your guide flag is "
                "on.\n");
        } else {
            colorize(BOLD_RED "Sorry, you can't X a new or unvalidated user!\n");
        }
        *to = 0;
        flush_input(1);
        return;
    ...
    for (i = 0; i < NXCONF &&
                (p->xconf[i].usernum != ouruser->usernum || !p->xconf[i].which);
         i++)
        ;
    ...
    if (tuser.xstat && !(p->f_twit)) {
        colorize(BOLD_RED "User has eXpress DISABLED.");
        if (ouruser->f_prog && override != 'w') {
            colorize(BOLD_GREEN " Do you wish to send the X anyway? -> ");
            if (yesno(-1) == YES) {
        ...
        if (override != 'W') {
            if (override != 'w') {
                ...
                colorize("  You are on user's 'enable' list.\n" BOLD_GREEN);
            } else {
                colorize("  Luckily for you it just doesn't matter!\n" BOLD_GREEN);
            }
        } else
            override = 'w';
    }
    ...
    if (mybtmp->xstat) {
        for (i = 0; i < NXCONF && (ouruser->xconf[i].usernum != tuser.usernum ||
    ...
    if (!**send_string) override = 'p';

    if (override == 'p') {
        if (!(buser = is_online(NULL, p, NULL)) || buser->nox < 0) {
            my_printf("%s has logged out.\n", p->name);
        } else if (!buser->nox) {
            my_printf("%s is not busy", p->name);
            if (buser->sleeptimes > 4) {
                my_printf(", but is idle for %02d:%02d", buser->sleeptimes / 60, buser->sleeptimes % 60);
            }
            my_printf(".\n");
        } else {
            my_printf("%s is busy.\n", p->name);
...
static void sendx(struct btmp *buser, struct user *touser,
    ...
    if (override != 'B' &&
        !((buser = is_online(NULL, touser, NULL)) && buser->nox >= 0)) {
    ...
    if (override == ' ' && buser->xstat && !(touser->f_twit)) {
        my_printf(
            "Sorry, %s turned off X messages before you finished your message!\n",
            touser->name);
        flush_input(1);
        return;
    }
    ...
    msg->xcurpos =
        ((unsigned char *)p + sizeof(int32_t) - xmsg) & ~(sizeof(int32_t) - 1);
...
void change_express(int cmd) {
    colorize("%s" BOLD_RED "eXpress messages %sABLED\n",
...
void change_beeps(void) {
    locks(SEM_USER);
    colorize(BOLD_RED "eXpress beeps %sABLED\n",
...
void old_express(void) {
    ...
    for (;;) {
        ...
        if (c) {
            if (!pos || displayx(n ? -pos : pos, i < 0 ? 0 : i,
                                 dir == BACKWARD ? &t : NULL, &prev, &next)) {
                if (!n) {
                    return;
                } else {
                    n = -1;
        ...
        my_printf("\nOld X message review  <N>ext (%s) <B>ack <S>top <#> -> ",
                  dir == BACKWARD ? "backward" : "forward");
        // c = 'N';
...
void get_syself_help(int cmd) {
    ...
    if (cmd == 'q') {
        my_printf(
            "\n\nPlease hit shift 'Q' (capital 'Q' not lowercase 'q') if you have "
    ...
    if (ouruser->f_twit) {
        my_printf(
            "\n\nSorry, you may not use this function while your BBS privileges "
            "are suspended.\nPlease Yell to the sysops for more information.\n");
        return;
    }
    ...
    if (!*curr_syself || !(buser = is_online(&btmp, NULL, curr_syself)) ||
        !btmp.elf || btmp.xstat) {
        if (*curr_syself) {
            my_printf(
        ...
        *curr_syself = 0;
        t = time(0);
        for (i = 0, n = (pid + t) % MAXUSERS; i < MAXUSERS;
             i++, n = (n == MAXUSERS - 1) ? 0 : n + 1) {
            if (bigbtmp->btmp[n].pid && bigbtmp->btmp[n].elf &&
                !bigbtmp->btmp[n].xstat && !bigbtmp->btmp[n].nox &&
                bigbtmp->btmp[n].pid != pid) {
                diff = 121 - abs(120 - (int)((t - bigbtmp->btmp[n].time) / 60));
    ...
    if (save < 0 && !*curr_syself)
        my_printf(
            "\nI'm sorry, no Guides are available at the moment.  You can hit 'y' "
            "to Yell\nyour BBS related question to the Sysops, and it will be "
            "answered as soon\nas possible.\n");
    else {
        ...
        if (!**send_string) {
            colorize(BOLD_RED "No message given, so nothing was sent.\n");
        } else {
...
static int syself_ok(const char *name) {
    ...
    for (i = 0;
         i < NXCONF &&
         (ouruser->usernum != up->xconf[i].usernum || up->xconf[i].which) &&
         (up->usernum != ouruser->xconf[i].usernum || ouruser->xconf[i].which);
         i++)
        ;
    freeuser(up);
    return (i < NXCONF ? 0 : 1);
...
void xbroadcast(void) {
    ...
    for (i = 0; i < XLENGTH && (!i || *send_string[i - 1]); i++) {
        get_string(client ? "" : ">", 78, send_string[i], i);
        if (!strcmp(send_string[i], "ABORT")) {
            colorize(BOLD_RED "Broadcast message aborted.\n");
            return;
        }

        if (ouruser->f_beeps && !strcmp(send_string[0], "BEEPS")) override = 'b';
    }
    if (!**send_string) {
        return;
    }
...
int xyell(struct user *up, unsigned char *p) {
    char nstr[8];
    int32_t pos = ouruser->xmaxpos;
    struct xheader *xh;
    int i;
    char *s;
    int num = xmsgnum - 1;
    ...
    for (xh = (struct xheader *)(void *)(xmsg + pos); pos; num--,
        (sender ? (pos = xh->sprev) : (pos = xh->rprev)),
        xh = (struct xheader *)(void *)(xmsg + pos)) {
        if (!xh->checkbit) {
            return (-1);
        }
        ...
        p += sprintf((char *)p, "\n%s %s (%s) from %s to %s at %s %s\n",
                     sender ? "---" : (xh->type == X_QUESTION ? "%%%" : "***"),
                     xh->type == X_QUESTION ? "Question" : "Message", nstr,
                     getusername(xh->snum, 1), getusername(xh->rnum, 1),
                     formtime(6, xh->time),
                     sender ? "---" : (xh->type == X_QUESTION ? "%%%" : "***"));
        ...
        if (p - savep > 50000) {
            p += sprintf((char *)p,
                         "\n\n(Some X messages were not included due to message size "
                         "limitations)\n\n");
            my_printf(
                "\n\n(Some X messages were not included due to message size "
                "limitations)\n\n");
...
void clean_xconf(struct user *tmpuser) {
    int i, j;
    int32_t num;
