...
void locks(int which) {
  ...
  while ((i = msem_lock(&msg->sem[which])) < 0 && errno == EINTR) {
    if (f_death == 2) {
      break;
    }
  }
...
void unlocks(int which) {
  ...
  while ((i = msem_unlock(&msg->sem[which])) < 0 && errno == EINTR) {
    if (f_death == 2) {
      break;
    }
  }
...
int add_loggedin(struct user *up) {
  uint8_t i;
  uint8_t j;
  pid_t p;
  uint8_t count = 0;
  ...
  for (;;) {
    if ((btmpindex = ouruser->btmpindex) >= 0) {
      locks(SEM_USER);
      if (!bigbtmp->btmp[btmpindex].pid ||
          bigbtmp->btmp[btmpindex].usernum != ouruser->usernum) {
        ouruser->btmpindex = -1;
      }
      unlocks(SEM_USER);
    }
    ...
    for (i = 0; i < MAXUSERS; i++) {
      if ((p = bigbtmp->btmp[i].pid) &&
          bigbtmp->btmp[i].usernum == up->usernum) {
        if (!guest) {
          unlocks(SEM_BTMP);
          if (kill(p, 0) < 0) {
          ...
          fflush(stdout);
          for (j = 0; j <= 15 && bigbtmp->btmp[i].pid == p &&
                      bigbtmp->btmp[i].usernum == up->usernum;
               j++) {
            if (j == 0) {
              kill(p, SIGQUIT);
              ...
              if (kill(p, SIGABRT) < 0) {
                errlog("Error %s forcing SIGABRT pid %d index %d",
                       strerror(errno), p, i);
              } else {
              ...
              remove_loggedin(p);
              locks(SEM_USER);
              if (ouruser->btmpindex == i) ouruser->btmpindex = -1;
              unlocks(SEM_USER);
            }
            my_putchar('.');
            bbs_sleep(2);
    ...
    mybtmp->elf =
        up->f_autoelf && up->f_elf && !up->f_restricted && !up->f_twit;
    mybtmp->connecting = 0;
    mybtmp->guest = guest;
    ...
    if (ouruser->btmpindex == -1) {
      ...
      unlocks(SEM_USER);
      errlog("btmpindex %d != ouruser->btmpindex %d", btmpindex,
             ouruser->btmpindex);
      ouruser->btmpindex = 0;
      // mybtmp->pid = 0;
      continue;
    ...
    if (count >= (MAXUSERS >> 5)) {
      my_printf(
          "\nI'm sorry, too many Guest users are logged in at the moment, "
          "please try again\nlater.\n\n");
      my_exit(10);
    }
...
void remove_loggedin(int p) {
  ...
  for (i = bigbtmp->users - 1; i >= 0; i--) {
    if (bigbtmp->btmp[bigbtmp->index[i]].pid == p) {
      locks(SEM_BTMP);
      if (bigbtmp->btmp[bigbtmp->index[i]].pid == p) {
        for (btmp = &bigbtmp->btmp[bigbtmp->index[i]]; i < bigbtmp->users - 1;
             i++) {
...
void reserve_slot(void) {
  ...
  if (bigbtmp->ghostcheck + 60 < newbtmp.time) {
    ...
    for (int i = 0; i < MAXUSERS; i++) {
      ...
      if (p != 0) {
        if (kill(p, 0) < 0) {
          errlog("Cleaned up ghost login of %s: %s", bigbtmp->btmp[i].name,
                 strerror(errno));
          remove_loggedin(p);
  ...
  for (;;) {
    // Find the index that has the smallest `eternal` value, where `pid` is
    // zero.
    ...
    if (bigbtmp->btmp[min_eternal_ix].pid == 0) {
      ...
      errlog("Reserving btmp slot %d for pid %d (eternal %i)", min_eternal_ix,
             bigbtmp->btmp[min_eternal_ix].pid, bigbtmp->eternal);
...
void validate_users(int what) {
  ...
  while (c != 'Q') {
    ...
    for (i = 0, count = 0, pick = -1; i <= msg->maxnewbie; i++) {
      if (msg->newbies[i].time && msg->newbies[i].time + 14400 < t && ++count &&
          !(skips[i >> 3] & 1 << (i & 7)) &&
          (pick < 0 || msg->newbies[pick].time > msg->newbies[i].time)) {
        pick = i;
      }
    }
    if (!count) {
    ...
    for (c = 'p';;) {
      switch (c) {
        case '?':
          my_printf(
              "\n\nChoices are <A>ccept <C>hange <D>elete <I>gnore <P>rofile "
              "<S>kip <Q>uit\n");
          break;
        ...
        case 'I':
          ...
          if (tuser->f_trouble) {
            my_printf(
                "\nA 'Trouble' user must be either deleted, accepted, or "
                "skipped.\n");
            c = 0;
            break;
          }
 ...
 */
void logout_user(struct user *tmpuser, struct btmp *btmp, int how) {
