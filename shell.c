...
char *get_name(const char *prompt, int quit_priv) {
  char *p;
  char c;
  int8_t upflag;
  int8_t fflag;
  ...
  for (;;) {
    ...
    for (;;) {
      ...
      if (c == BS || c == CTRL_X || c == CTRL_W || c == CTRL_R || c == SP ||
          (c >= 'A' && c <= 'Z') || (c >= 'a' && c <= 'z') ||
          (c >= '0' && c <= '9' && quit_priv == 3)) {
        invalid = 0;
      } else {
      ...
      do
        ...
        } else if (p < &pbuf[!quit_priv || quit_priv == 3 ? MAXNAME
                                                          : MAXALIAS] &&
                   c != BS && c != CTRL_X && c != CTRL_W) {
          fflag = 0;
          if (upflag && c >= 'a') {
            c -= 32;
          }
 ...
 * do_login()
 *
 * Prompt the user for his username and password and then call login_user Set
 * the global variable ouruser
 */
...
void do_login(void) {
  ...
  for (;;) {
    ...
    if (strcmp(name, "New")) {
      ...
      if (!tmpuser || !(ouruser = login_user(name, pas))) {
        ...
        if (ouruser->f_deleted) {
          if (ouruser->f_namechanged) {
            my_printf(
                "\a\nThis account has been marked for deletion because of a "
                "request to change the\nusername from '%s' to '%s'.\n\nYou may "
                "login as '%s' using the same password.\n\n",
                ouruser->name, ouruser->reminder, ouruser->reminder);
          } else {
            my_printf(
                "\a\nThis account has been marked for deletion, either through "
                "your choice or\nbecause you violated BBS rules by doing "
                "something such as providing\nobviously bogus profile info.  "
                "You will be logged off.\n\n");
          }
          my_exit(10);
        } else if (ouruser->f_inactive) {
          my_printf(
              "You seem to have been denied access to the message system.\n");
          // my_printf("Please contact ISCA (e-mail address
          // bbs@bbs.isca.uiowa.edu) for more.\n");
          my_exit(10);
        }
        ...
        if (ouruser->f_noclient) {
          my_printf(
              "\n\nYou have been disallowed use of the BBS client, you must "
              "login using telnet.\n\n");
          my_exit(10);
        }

        checked_snprintf(
            temp, sizeof(temp), "%s %s%s%s/%d", client ? "CLIENT" : "LOGIN",
            ARGV[1] && ARGV[2] ? ARGV[2] : "", ARGV[1] && ARGV[2] ? "@" : "",
            ouruser->remote, mybtmp->remport);
        logevent(temp);

        ++ouruser->timescalled;

        if (!guest) {
          my_printf("This is call %d.  ", ouruser->timescalled);

          if (users > 1) {
            my_printf("There are %d users.\n", users);
          } else {
            my_printf("There is 1 user.\n");
          }
        }
        if (ouruser->f_aide) {
          validate_users(0);
        ...
        if (*ouruser->reminder) {
          my_printf("\n\aREMINDER:\n%s\n\n", ouruser->reminder);
          my_printf(
              "Please hit 'Y' to acknowledge having seen this reminder -> ");
          get_single_quiet("yY");
        }
      ...
      my_printf(
          "\n\nSorry, there was some problem setting up your BBS "
