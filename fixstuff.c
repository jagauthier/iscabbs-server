...
int num_compare(const void *_a, const void *_b) {
  int a = *(int *)_a;
  int b = *(int *)_b;
  if (usernumbers[a] < usernumbers[b]) return -1;
  if (usernumbers[a] == usernumbers[b]) return 0;
  return 1;
}

int name_compare(const void *_a, const void *_b) {
  int a = *(int *)_a;
  int b = *(int *)_b;
...
int main() {
  ...
  qsort(&(master.num[which]), totalusers, sizeof(int), num_compare);
  qsort(&(master.name[which]), totalusers, sizeof(int), name_compare);
  ...
  int current = -1;
  for (i = 0; i < totalusers; i++) {
    if (prev > -1 && strcmp(usernames[prev], usernames[array[i]]) == 0) {
