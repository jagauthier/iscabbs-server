...
void change_setup(struct user *workuser) {
  char c = -1;
  int8_t i;
  char chflag;
  struct user *up = NULL;
  ...
  for (;;) {
    ...
    if (strchr("dn", c)) {
      ...
      my_printf(
          "\n\nYou are not using the BBS client.  For more information on the "
    ...
    switch (c) {
      ...
      case 'Q':
        my_printf(
            "\n\nIf you have a question related to this BBS to ask a Guide, "
            "you need to exit the\nconfig menu (the menu you are currently in) "
            "first.  To do so, press the space\nbar.  You may then hit shift "
            "'Q' (capital 'Q' not lowercase 'q') to ask your\nquestion.\n");
        break;
...
static void change_addr(struct user *tuser, int chflag) {
  ...
  my_printf(
      "\nEnter your address information.  To leave a entry alone, just hit "
      "return.\n\n");
  ...
  char *phone = my_sprintf(
      NULL, "Phone number (including all prefixes!) [%s]: ", tuser->phone);
...
static void change_aide_info(struct user *tuser) {
  ...
  if (*tuser->aideinfo) {
    my_printf(
        ...
        "this? (Y/N) -> ",
        tuser->name, tuser->aideinfo);
  } else {
    my_printf("\nDo you wish to add a line of sysop info for %s? (Y/N) -> ",
              tuser->name);
  }
  if (yesno(-1)) {
...
static void change_anonymous(struct user *tuser, int chflag) {
  ...
  my_printf(
      "\nYou have the option of hiding some or all of your personal "
      "information (name,\naddress, phone, and e-mail) from others on this "
      "BBS.\n\n");
  if (chflag) {
    ...
    my_printf(
        "<H>ide all  <U>nhide all  <S>elect individual items  <Q>uit -> ");
  ...
  switch (c) {
    ...
    case 'H':
      ...
      locks(SEM_USER);
      tuser->an_name = tuser->an_addr = tuser->an_location = tuser->an_phone =
          tuser->an_mail = tuser->an_www = tuser->an_site = 1;
      tuser->an_all = 0;
      unlocks(SEM_USER);
      break;
    ...
    case 'U':
      ...
      locks(SEM_USER);
      tuser->an_name = tuser->an_addr = tuser->an_location = tuser->an_phone =
          tuser->an_mail = tuser->an_www = tuser->an_site = 0;
      tuser->an_all = 0;
      unlocks(SEM_USER);
      break;
...
static void change_reminder(struct user *tuser) {
  ...
  my_printf(
      "\nEnter a single line of what you want yourself reminded of upon "
...
static void change_name(struct user *workuser) {
  ...
  if (!*name) {
    ...
    my_printf("\nName too long.\n");
  } else if (!strcmp(name, "new") || strstr(name, "sysop") ||
             strstr(name, "moderator") || strstr(name, "isca") ||
             !strcmp(name, "guest") || (tmpuser = getuser(name))) {
    if (tmpuser != workuser) {
      freeuser(tmpuser);
    }
    my_printf("\nName already in use.\n");
    ...
    if (yesno(-1)) {
      ...
      checked_snprintf(work, sizeof(work), "NAMECHANGE: %s to %s",
                       workuser->name, name);
      logevent(work);
...
void do_verify(struct user *workuser, int ask) {
  if (ask) {
...
static void show_verified(struct user *workuser) {
  my_printf(
      "\nReal name: %s\nAddress: %s\nCity: %s\nState: %s\nZIP: %s\nPhone: "
      "%s\nEmail: %s\n\n",
      workuser->A_real_name, workuser->A_addr1, workuser->A_city,
      workuser->A_state, workuser->A_zip, workuser->A_phone, workuser->A_mail);
}
...
static void ooptions(struct user *tuser) {
  ...
  my_printf(
      "Be asked if you are on an ANSI-compatible terminal when you login? -> ");
...
static void foptions(struct user *tuser) {
  ...
  if (yesno(tuser->f_badinfo) != tuser->f_badinfo) {
    locks(SEM_USER);
    tuser->f_restricted =
        (tuser->f_badinfo ^= 1) | tuser->f_newbie | tuser->f_duplicate;
    unlocks(SEM_USER);
    if (!tuser->f_badinfo) {
      do_verify(tuser, 1);
    }
  ...
  if (yesno(tuser->f_newbie) != tuser->f_newbie) {
    locks(SEM_USER);
    tuser->f_restricted =
        (tuser->f_newbie ^= 1) | tuser->f_badinfo | tuser->f_duplicate;
    unlocks(SEM_USER);
  }

  my_printf("Make user a guide? -> ");
  if ((answer = yesno(tuser->f_elf)) && !tuser->f_elf &&
      (tuser->timescalled < 100 || tuser->posted < 100)) {
    my_printf("  %s has %d calls, %d posts.  Are you sure? (Y/N) -> ",
              tuser->name, tuser->timescalled, tuser->posted);
    answer = yesno(-1);
  }
  ...
  if (yesno(tuser->f_duplicate) != tuser->f_duplicate) {
    locks(SEM_USER);
    tuser->f_restricted =
        (tuser->f_duplicate ^= 1) | tuser->f_newbie | tuser->f_badinfo;
    unlocks(SEM_USER);
  }
  ...
  if (tuser->f_inactive) logout_user(tuser, NULL, 0);
...
static void xoptions(struct user *tuser) {
  my_printf(
      "Options\n\nHave eXpress messages turned OFF when you first enter the "
      "BBS? -> ");
  ...
  for (;;) {
    ...
    if (c == 'L') {
      ...
      if (!tmpuser->xconf[0].usernum)
        ...
        for (i = 0; i < NXCONF; i++) {
          if (tmpuser->xconf[i].usernum &&
              (name = getusername(tmpuser->xconf[i].usernum, 0)) &&
              !tmpuser->f_invisible) {
      ...
      my_printf("%s\n\nUser to %s enable/disable list -> ",
                c == 'A' ? "Add" : "Delete",
                c == 'A' ? "Add to" : "Delete from");
      name = get_name("", 2);
      if (!*name) {
        continue;
      }
...
static struct user *change_user(void) {
  ...
  if (!name || !(tmpuser = getuser(name)) ||
      (!strcmp(name, "Guest") && !ouruser->f_prog)) {
    if (tmpuser) {
...
void doingchange(const char *doing) {
  size_t size;
  size_t count;
...
static void do_bigzap(struct user *tmpuser) {
  ...
  my_printf(
      "Would you like to <z>ap or <u>nzap all public rooms?  (<enter> to "
      "cancel) -> ");
  ...
  if (c == 'U') {
    for (i = 2; i < MAXROOMS; i++) {
      if (msg->room[i].flags & QR_INUSE && !(msg->room[i].flags & QR_PRIVATE) &&
...
static void change_vanityflag(struct user *tmpuser) {
  ...
  if (*flag) {
    locks(SEM_USER);
    checked_snprintf(tmpuser->vanityflag, sizeof(tmpuser->vanityflag), "%s",
                     flag);
    unlocks(SEM_USER);
  }
}
