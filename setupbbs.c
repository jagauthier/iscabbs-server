...
static void init_file(const char *path, size_t size, mode_t mode) {
  ...
  if (fd == -1) err(1, "init_file: failed to open '%s' for writing", path);

  // resize the file, filling it with zeros.
  int rc = ftruncate(fd, size);
  if (rc == -1) err(1, "init_file: ftruncate failed for file '%s'", path);

  close(rc);
}

static void setupbtmp(void) {
  init_file(TMPDATA, sizeof(struct bigbtmp), 0660);
  printf("btmp setup complete.\n");
}

static void setupmsgmain(void) {
  init_file(MSGMAIN, MM_FILELEN, 0660);
  printf("msgmain setup complete.\n");
}

static void setupmsgdata(void) {
  ...
  if (rc == -1) err(1, "setupmsgdata: failed to write file '%s'", MSGDATA);
...
static void setupuserdata(void) {
  init_file(USERDATA,
            sizeof(struct userdata) + sizeof(struct user) * MAXTOTALUSERS,
            0660);
  printf("userdata setup complete\n");
}

static void setupxmsgdata(void) {
  init_file(XMSGDATA, XMSGSIZE, 0660);
  printf("xmsgdata setup complete\n");
}

static void setupvoteinfo(void) {
