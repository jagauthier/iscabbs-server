...
void runbbs(int16_t s) {
  ...
  while (q->qp) {
    if ((q->limit >= 0 && q->limit <= q->forks - q->reaps) ||
        q->startable <= 0) {
      if (q->qflag || s)
        i = q->qp - 1;
      else {
      ...
      if (s > 0 && !LOCAL(q->qt[s].addr) && !q->qt[s].wasinq) {
        ...
        q->qt[s].wasinq = 1;
        if (!q->qt[s].client) {
          len = checked_snprintf(str, sizeof(str), IFYOUHADCLIENT,
                                 q->qt[s].wouldbe);
          ssend(s, str, len);
        }
      }
      checked_snprintf(date, sizeof(date), "%02d:%02d:%02d", q->ltm->tm_hour,
                       q->ltm->tm_min, q->ltm->tm_sec);
      for (; i >= 0; i--) {
        if ((!s && q->qt[q->qindex[i]].qlo <= i) ||
            (s < 0 && q->qindex[i] != -s) ||
            (s <= 0 && q->qt[q->qindex[i]].login >= 10) ||
            (s > 0 && q->qindex[i] != s) || LOCAL(q->qt[q->qindex[i]].addr) ||
            q->qt[q->qindex[i]].login == -14) {
          continue;
        }
        if (!i || !q->qt[q->qindex[i]].qlo) {
          len = checked_snprintf(str, sizeof(str), ATFRONT, date,
                                 (q->t - q->qt[q->qindex[i]].conn) / 60,
                                 (q->t - q->qt[q->qindex[i]].conn) % 60);
        } else if (i == 1 || q->qt[q->qindex[i]].qlo == 1) {
          len = checked_snprintf(str, sizeof(str), ONEAHEAD, date,
                                 (q->t - q->qt[q->qindex[i]].conn) / 60,
                                 (q->t - q->qt[q->qindex[i]].conn) % 60,
                                 bigbtmp->users, q->qp);
        } else {
          len = checked_snprintf(
              str, sizeof(str), MANYAHEAD, date,
              (q->t - q->qt[q->qindex[i]].conn) / 60,
              (q->t - q->qt[q->qindex[i]].conn) % 60,
              i <= q->qt[q->qindex[i]].qlo ? i : q->qt[q->qindex[i]].qlo,
              bigbtmp->users, q->qp);
        }
        if (i < q->qt[q->qindex[i]].qlo) {
      ...
      if (q->qt[q->qindex[0]].login != -14 &&
          !LOCAL(q->qt[q->qindex[0]].addr)) {
        break;
      }
    } else if (s < 0 && (q->qt[q->qindex[0]].login != -14 &&
                         !LOCAL(q->qt[q->qindex[0]].addr))) {
      break;
    }

    for (pick = 0; pick < q->qp; pick++) {
      if (q->qt[i = q->qindex[pick]].conn &&
          (q->qt[i].state == TS_DATA || q->qt[i].state == TS_CR) &&
          !q->qt[i].ncc) {
        if (pick >= q->qp ||
            (q->qt[i].login != -14 && !LOCAL(q->qt[i].addr) &&
...
#else
      else if (q->qt[i].last + 300 < q->t) {
        syslog(LOG_INFO, "Time difference %ld for %08x", q->t - q->qt[i].last,
               q->qt[i].addr.s_addr);
      }
#endif
    }
    ...
    checked_snprintf(newenv[0] = lines, sizeof(lines), "ROWS=%d",
                     q->qt[y].rows);
    if (q->qt[y].login < 0) {
      checked_snprintf(newenv[1] = bbsname, sizeof(bbsname), "BBSNAME=%s",
                       q->qt[y].name);
    } else {
      newenv[1] = 0;
    }
    ...
    if ((i = fork())) {
      ...
      if (fcntl(y, F_SETFL, 0) < 0) logfatal("fcntl: %m");
      { closelog(); }
      ...
      if (y > 0) {
        ...
        if (select(s + 1, &rfds, NULL, 0, &tv) > 0) {
          ...
          if ((p = strstr(str, "USERID"))) {
            ...
            for (p = identname + sizeof identname - 2; !*p || isspace(*p);
                 p--) {
              *p = 0;
            }
            if (*remoteusername && strcmp(remoteusername, identname)) {
              syslog(LOG_INFO, "Liar, %s@%s claimed to be %s", identname, host,
                     remoteusername);
            }
