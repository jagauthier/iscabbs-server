  ...
  while (q->qt[x].ncc > 0) {
    ...
    switch (q->qt[x].state) {
      ...
      case TS_DATA:
        ...
        if (++q->qt[x].acc > 80 + (q->t - q->qt[x].conn) / 30 ||
            q->qt[x].acc < 0) {
      ...
      gotiac:
        switch (c) {
          ...
          case CLIENT:
            syslog(LOG_NOTICE, "Old client from %s",
                   inet_ntoa(*(struct in_addr *)&q->qt[x].addr));
...
static void send_will(int option, int init, int x) {
  if (init) {
