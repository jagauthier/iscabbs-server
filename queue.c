...
void bbsqueue(int dofork) {
  int16_t x;
  int i;

  // sigblock(sigmask(SIGCLD) | sigmask(SIGALRM) | sigmask(SIGHUP) |
  // sigmask(SIGUSR2) | sigmask(SIGTERM) | sigmask(SIGQUIT));
  ...
  for (;;) {
    ...
    if (i) {
      if (i == EINTR) {
        if (f_term) do_quit();
        if (f_qalarm) do_ring();
        if (f_reread) do_reread();
        if (f_quit) do_setup();
        if (f_restart) do_restart();
        runbbs(0);
        continue;
      } else
        logfatal("select: %m");
    ...
    while (FD_ISSET(sfd, &q->fds)) {
      ...
      q->socks++;
      if (q->lockout && (sa.sin_addr.s_addr >= 0x80ff3865UL &&
                         sa.sin_addr.s_addr <= 0x80ff3868UL)) {
        if (((!q->ltm->tm_wday || q->ltm->tm_wday == 6) &&
             q->ltm->tm_hour >= 12) ||
            q->ltm->tm_hour >= 14 ||
            (q->ltm->tm_wday && q->ltm->tm_wday < 6 && !q->ltm->tm_hour)) {
      ...
      *q->qt[x].remoteusername = 0;
      ...
      for (size_t i = 0; i < sizeof q->qt[0].options; i++) {
        q->qt[x].options[i] = q->qt[x].do_dont_resp[i] =
            q->qt[x].will_wont_resp[i] = 0;
      }
      q->qt[x].ncc = 0;
    ...
    for (x = 4; x < MAXQ; x++) {
      ...
      if (FD_ISSET(x, &q->fds)) {
        while ((q->qt[x].ncc = recv(x, q->qt[x].netibuf,
                                    sizeof q->qt[x].netibuf, 0)) < 0 &&
               errno == EINTR)
          ;
        if (q->qt[x].ncc <= 0) {
          drop(x);
