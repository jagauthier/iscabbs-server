...
static void editdesc(void) {
  ...
  bool dummy; /* readmsg() needs this: returns YES/NO */
  int8_t err; /* makemessage returns this */
  ...
  if (choice == 'B' || choice == 'F') {
    ...
    } else if (!(tmpuser = getuser(cp)) || tmpuser->f_invisible) {
      if (tmpuser) {
        freeuser(tmpuser);
      }
...
static void editroom(void) {
  ...
  if (ouruser->f_admin) {
    ...
    my_printf(
        "New forum type <1>pub <2>guessname <3>passworded <4>inv-only -> ");
    opt = get_single_quiet("1234");
...
static void invite(void) {
  ...
  if (*uname) {
    ...
    if (tmpuser->generation[curr] == msg->room[curr].gen ||
        (!(msg->room[curr].flags & QR_PRIVATE) &&
...
static void kickout(void) {
  ...
  if (*uname) {
    if (!(tmpuser = getuser(uname)) || tmpuser->f_invisible) {
      ...
      if ((tmpuser->generation[curr] == ((msg->room[curr].flags & QR_PRIVATE)
                                             ? TWILIGHTZONE
                                             : RODSERLING)) &&
          tmpuser->forget[curr] == TWILIGHTZONE) {
        freeuser(tmpuser);
      ...
      locks(SEM_USER);
      tmpuser->generation[curr] = ((msg->room[curr].flags & QR_PRIVATE) &&
                                   !(msg->room[curr].flags & QR_GUESSNAME))
                                      ? TWILIGHTZONE
                                      : RODSERLING;
