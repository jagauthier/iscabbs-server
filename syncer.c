...
int bbssync(int init) {
  int32_t oldcputime[9];
  time_t t;
  int flags;
  int32_t curpos;
  FILE *fp;
  ...
  if (init) {
    ...
    for (i = 0; i < 7; i++) msem_init(&msg->sem[i], -1);
  ...
  for (oldcputime[0] = 0;;) {
    ...
    if (oldcputime[0]) {
      //      fprintf(fp, "%02d/%02d %02d:%02d  %4d/%3d  user %02d%%/%02d%%  sys
      //      %02d%%  idle %02d%%/%02d%%  %5.2f/%5.2f/%5.2f\n", tm->tm_mon + 1,
      //      tm->tm_mday, tm->tm_hour, tm->tm_min, bigbtmp->users,
      //      bigbtmp->queued, (pst->psd_cpu_time[0] - oldcputime[0]) / 60,
      //      (pst->psd_cpu_time[1] - oldcputime[1]) / 60, (pst->psd_cpu_time[2]
      //      - oldcputime[2]) / 60, (pst->psd_cpu_time[3] - oldcputime[3]) /
      //      60, (pst->psd_cpu_time[4] - oldcputime[4]) / 60,
      //      pst->psd_avg_1_min, pst->psd_avg_5_min, pst->psd_avg_15_min);
      fflush(fp);
    }
    //    for (f = 0; f < 5; f++)
    ...
    if (curpos - trim * 1024 * 1024 >= 0) {
      if (curpos + 256 * 1024 >= msg->xmsgsize) {
        msync(
            (caddr_t)xmsg + (curpos + 256 * 1024 - msg->xmsgsize),
            curpos - trim * 1024 * 1024 - (curpos + 256 * 1024 - msg->xmsgsize),
            MS_INVALIDATE);
      } else {
    ...
    if (curpos + 256 * 1024 < msg->xmsgsize) {
      if (trim * 1024 * 1024 - curpos >= 0) {
        msync(
            (caddr_t)xmsg + curpos + 256 * 1024,
            msg->xmsgsize - curpos - 256 * 1024 - (trim * 1024 * 1024 - curpos),
            MS_INVALIDATE);
      } else {
        msync((caddr_t)xmsg + curpos + 256 * 1024,
              msg->xmsgsize - curpos - 256 * 1024, MS_INVALIDATE);
      }
    }
    ...
    if (curpos - trim * 1024 * 1024 >= 0) {
      if (curpos + 256 * 1024 >= 61036 * 4096) {
        msync(
            (caddr_t)msgstart + (curpos + 256 * 1024 - 61036 * 4096),
            curpos - trim * 1024 * 1024 - (curpos + 256 * 1024 - 61036 * 4096),
            MS_INVALIDATE);
      } else {
    ...
    if (curpos + 256 * 1024 < 61036 * 4096) {
      if (trim * 1024 * 1024 - curpos >= 0) {
        msync(
            (caddr_t)msgstart + curpos + 256 * 1024,
            61036 * 4096 - curpos - 256 * 1024 - (trim * 1024 * 1024 - curpos),
            MS_INVALIDATE);
      } else {
        msync((caddr_t)msgstart + curpos + 256 * 1024,
              61036 * 4096 - curpos - 256 * 1024, MS_INVALIDATE);
      }
    }

    {
      char *corepath =
          my_sprintf(NULL, ROOT "/core/bbs/core-%02d%02d%02d%02d",
    ...
    {
      char *corepath =
          my_sprintf(NULL, ROOT "/core/bbsqueued/core-%02d%02d%02d%02d",
                     tm->tm_mon + 1, tm->tm_mday, tm->tm_hour, tm->tm_min);
