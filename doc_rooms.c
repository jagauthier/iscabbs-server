...
bool findroom(void) {
  ...
  for (int i = 0; i < MAXROOMS; ++i) {
    if (strstr(msg->room[i].name, rname) && (msg->room[i].flags & QR_INUSE) &&
        (!(msg->room[i].flags & QR_PRIVATE) ||
         msg->room[i].gen == ouruser->forget[i] ||
         msg->room[i].gen == ouruser->generation[i])) {
      if (ouruser->generation[i] == RODSERLING) {
...
void loadroom(void) {
  ...
  if (curr == MAIL_RM_NBR) {
    /** This j loop will never get executed */
    int j;
    for (j = 0; j < MSGSPERRM - MAILMSGS; j++) {
      room->num[j] = room->pos[j] = room->chron[j] = 0;
    }
    for (uint8_t i = 0; j < MSGSPERRM; i++, j++) {
      room->num[j] = room->chron[j] = ouruser->mr[i].num;
      room->pos[j] =
          ouruser->mr[i].pos < 0 ? -ouruser->mr[i].pos : ouruser->mr[i].pos;
    }
...
bool nextroom(void) {
  ...
  if (checkmail(ouruser, NOISY) >
      0) { /* Forced into mailroom ASAP after mail arrives */
    curr = MAIL_RM_NBR;
    return (YES);
  }
...
void readroom(int cit_cmd) {
  ...
  while ((rm_msg_nbr < MSGSPERRM) && (rm_msg_nbr >= 0) && (stop == FALSE)) {
    /*
    ...
    */
    ...
    error = readmessage(msgstart + room->pos[rm_msg_nbr], auth, name,
                        (cit_cmd == 'N'), savedid = room->num[rm_msg_nbr]);
    ...
    for (exitloop = 0, chr = 1; !exitloop;) {
      ...
      if (ouruser->f_admin ||
          (ouruser->usernum == msg->room[curr].roomaide && !ouruser->f_twit)) {
        chr = get_single_quiet(
            "0123456789aABCdD\005eEHILNpPqQrRSTvwWxX\027\030yY ?%\"");
      } else if (*auth || curr == MAIL_RM_NBR) {
        chr = get_single_quiet(
            "0123456789aABCdDeEHILNpPqQrRSTvwWxX\027\030yY ?%\"");
      } else {
        chr = get_single_quiet(
            "0123456789aABCeEHILNpPqQrRSTvwWxX\027\030yY ?%\"");
      }
      ...
      switch (chr) {
        ...
        case 'B':
          ...
          if ((cit_cmd == 'O') || (cit_cmd == 'R')) {
            cit_cmd = 'F';
          } else if ((cit_cmd == 'N') || (cit_cmd == 'F') || (cit_cmd == '#')) {
            cit_cmd = 'R';
          }
          exitloop = TRUE;
          break;
        ...
        case '%':
          if (ouruser->f_elf && !ouruser->f_restricted && !ouruser->f_twit) {
            if (mybtmp->xstat && !mybtmp->elf) {
              ...
              my_printf(
                  "\n\nYou are now marked as being available to help "
                  "others.\n");
            } else {
              my_printf(
  ...
  switch (cit_cmd) {
    ...
    case '#':
      ...
      *searchkey = atol(nbr_str);
      if (*searchkey < 0L) *searchkey = abs(*searchkey);
    ...
    case '-':
      *dir = FORWARD;
      ...
      nbr = atoi(nbr_str);
      if (nbr < 1) nbr = 0;
      if (nbr > (MSGSPERRM - 1)) nbr = MSGSPERRM - 1;
